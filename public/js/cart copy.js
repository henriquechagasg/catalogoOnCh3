...
const deleteB = document.querySelectorAll('.delete-btn')
const clientOrderInput = document.querySelector('#client-order')
const modalClientOrderInput = document.querySelector('#modal-client-order')
clientOrderInput.value = JSON.stringify(products);
modalClientOrderInput.value = JSON.stringify(products);
...
function addCartTotals(){
    const totalsArea = document.querySelectorAll('.cart-totals')
    const [ sumAllProducts, sumAllPrice ] = getProductSum(products);
    totalsArea.forEach(element => {
        let cartTotalQuantity = document.createElement('h2')
        cartTotalQuantity.innerText = `Total de Peças: ${sumAllProducts}`
        let cartTotalPrice = document.createElement('h2')
        cartTotalPrice.innerText = `Valor Total: R$${sumAllPrice}`
        element.append(cartTotalQuantity, cartTotalPrice)
    })
}
...
function generateProducts(products){
    ...
    products.forEach(product => {
        ...
        if (product == products[products.length - 1]){
            eachCartProduct.setAttribute('style', 'margin: 0 0 200px 0')
        }
...
//     const modalContainer = document.querySelector('.modal-close-cart-container');
//     const modal = document.querySelector('.modal-close-cart-card')
//     const clickToOpen = document.querySelector('.click-to-open-modal') 
//     addGlobalEventListener('click', '.click-to-open-modal', e => {
//         if (!modalContainer.classList.contains('open')){


//             modalContainer.classList.add('open');
//             modal.classList.add('active-modal');
//             [...clickToOpen.children][0].setAttribute('style', 'display: none;');
//             [...clickToOpen.children][1].setAttribute('style', 'display: none;');


//         } else {
//             modalContainer.classList.remove('open');
//             modal.classList.remove('active-modal');
//             [...clickToOpen.children][0].setAttribute('style', 'display: block;');
//             [...clickToOpen.children][1].setAttribute('style', 'display: block;');
//         }
//     })
// }
